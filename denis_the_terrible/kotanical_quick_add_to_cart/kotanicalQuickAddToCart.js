...
<style>
    ...
    @media only screen and (max-width: 440px) {
      ...
      .twik-cart-checkout-btn-a, .twik-cart-checkout-btn {
        width: 100%;
      }
...
let updateShippingText;
let freeShippingAmount = 40;
...
function mainJS() {
  ...
  getCart = () => {
    return JSON.parse(Get("/cart.json"));
  };

  clearAndHideProductList = () => {
    productList.innerHTML = "";
    $(productList).hide();
    $(".twik-cart-footer").css({ visibility: "hidden" });
    $(cartEmptyDiv).show();
  ...
  populateProductList = () => {
    ...
    for (let item of items) {
      ...
      updateShippingText(total_price_CENTS / 100);
    }
  };
  ...
  updateShippingText = (currentTotal) => {
    document.querySelector(".twik-cart-shipping").innerText =
      currentTotal < freeShippingAmount
        ? `Spend €${
            freeShippingAmount - currentTotal
  ...
  document
    .querySelector(".twik-cart-close-wrapper")
...
function replace_addtc_buttons(selector) {
  for (let prod_index in document.querySelectorAll(selector)) {
    let prod = document.querySelectorAll(selector)[prod_index];
    ...
    let prod_form = prod.querySelector("form");
    prod_form.querySelector(`input[name="id"]`);
    ...
    let addToCartBtn = itemContent.querySelector(".twik-add-to-cart-btn");

    addToCartBtn.parentElement.addEventListener("click", (e) => {
      e.stopPropagation();
    });

    addToCartBtn.addEventListener("click", (e) => {
      e.preventDefault();
      addToCartButtonClick(prod, data_url);
    });
  }
}
...
function create_addtc_buttons(selector) {
  for (let prod_index in document.querySelectorAll(selector)) {
    ...
    let addToCartBtn = prod.querySelector(".twik-add-to-cart-btn");
...
function addToCartButtonClick(prod, data_url) {
  prod.querySelector(".twik-add-to-cart-btn").style.display = "none";
  prod.querySelector(".twik-adding-message").style.display = "block";
  ...
  fetch(data_url)
    ...
    .then((res) => {
      find_and_add_product(res);
      prod.querySelector(".twik-adding-message").style.display = "none";
      prod.querySelector(".twik-thankyou-message").style.display = "block";
      setTimeout(() => {
    ...
    .catch((error) => {
      console.log(`add to cart POST request failed; error: ${error}`);
      setTimeout(() => {
        prod.querySelector(".twik-adding-message").style.display = "none";
        prod.querySelector(".twik-add-to-cart-btn").style.display = "block";
      }, 1000);
...
function find_and_add_product(json_data) {
  let variant_id = json_data["product"]["variants"][0]["id"];

  fetch(`/cart/add.js?id=${variant_id}&quantity=1`, {
    method: "POST",
  })
    .then((response) => {
      if (response.ok) {
...
document.head.insertAdjacentHTML(
  "beforeend",
  `
<style id="twik-add-to-cart-buttons">
.twik-add-to-cart-btn-wrapper {
  display: flex;
  justify-content: center;
  align-items: center;
  flex-direction: column;
...
if (check_for_addtc(page_products_selector)) {
  replace_addtc_buttons(page_products_selector);
} else {
